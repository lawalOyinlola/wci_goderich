...
function Calendar({
  ...
  const defaultClassNames = {
    button_next: cn(
      ...
      "size-6 p-4 text-muted-foreground/80 hover:text-foreground hover:bg-transparent"
    ),
    button_previous: cn(
      buttonVariants({ variant: "ghost" }),
      "size-6 p-4 text-muted-foreground/80 hover:text-foreground hover:bg-transparent"
  ...
  const defaultComponents = {
    Chevron: (props: {
      ...
      if (props.orientation === "left") {
        return (
          <CaretLeftIcon size={8} weight="bold" {...props} aria-hidden="true" />
        );
      }
      return (
        <CaretRightIcon size={8} weight="bold" {...props} aria-hidden="true" />
      );
    },
  };
